...
def main():
    ...
    class SaveEveryNEpoch(tf.keras.callbacks.Callback):
        def __init__(self, n=5, ckpt_dir="checkpoints"):
            super().__init__()
            self.n = n
        ...
        def on_epoch_end(self, epoch, logs=None):
            if (epoch + 1) % self.n == 0:
                ep = epoch + 1
                weights_path = self.ckpt_dir / f"diffusion_weights_epoch{ep:03d}.h5"
    ...
    save_cb = SaveEveryNEpoch(n=5, ckpt_dir="./checkpoints")
